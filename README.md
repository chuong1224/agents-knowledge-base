...
- **Integrity lamps** (`/integrity`): what is actually *broken* — dangling `[[wikilinks]]`, broken `![[embeds]]`, `[[Note#Heading]]` anchors that no longer match, orphaned images/videos, notes missing required frontmatter fields, and binary attachments that were never summarised in the note. Click any finding to open the note right where it is
...
- **Sức khoẻ vault:** 🩺 *truy xuất* (`/insight`) — note nóng tuần này so với tuần trước, note đang nguội đi, phân bố tuổi lần đụng cuối, note chưa bao giờ vào đường truy xuất, cụm ít kết nối trên đồ thị note–note, coverage theo khu vực (kèm CLI `python insight.py --report` sinh note báo cáo); 🧪 *toàn vẹn* (`/integrity`) — wikilink gãy, nhúng gãy, anchor lệch heading, ảnh/video mồ côi, note thiếu trường frontmatter bắt buộc, file nhị phân chưa được tóm tắt trong note. Click một mục là mở đúng note đó. Chạy tay `python integrity.py` **trả exit 1 khi có lỗi** (cắm thẳng vào pre-commit/CI). Hai check contract đọc luật từ file quy tắc máy-đọc của chính vault, không có thì tự tắt êm — 4 check cấu trúc vẫn chạy.
