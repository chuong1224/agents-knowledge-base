...
- **Đọc & tìm:** click node đọc note ngay (wikilink, ảnh, backlink); click/Enter ảnh mở lightbox nội bộ với 100% tự nhiên, fit 80vw × 80vh, zoom neo con trỏ, pan clamp và menu copy/link/mở/tải có fallback đúng; cây thư mục kéo-giãn, `Ctrl+P` tìm tên / `#tag` / nội dung không dấu, tab + 2 pane + ghim + lịch sử đọc (persist).
